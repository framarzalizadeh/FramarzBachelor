...
\item Compiliere die Datei \texttt{thesis.tex} entweder aus der Kommandozeile oder aus dem \LaTeX-Editor heraus nach \gls{ac:pdf} mittels \texttt{pdflatex}. Es sollten keine Fehler auftreten und ein \gls{ac:pdf} erzeugt werden.
\item Passe die Konfiguration in \texttt{configuration/configuration.tex} an (siehe Abschnitt \ref{sec:configurations}).
\item Compiliere die Datei \texttt{thesis.tex} erneut. Es sollte ein \gls{ac:pdf} mit den neuen Einstellungen erzeugt werden.
