...
Dieses Kapitel beschreibt die technische Umsetzung des in Kapitel~4 konzipierten 
Systems. Zunächst wird die Implementierung des Chatbots über Make.com-Szenarien 
erläutert. Anschließend folgt die Datenhaltung mit Airtable und die Web-Anwendung 
ClientHub. Darauf aufbauend werden automatische Dokumentenerstellung in Google 
Docs, Ablagestrategie in Google Drive sowie die ClientHub WebApp mit Frontend, 
Edge Functions und \gls{ac:api}-Anbindungen detailliert beschrieben.

\section{Implementierung des Chatbots (make.com‑Szenarien)}

Das in \autoref{chap:konzeption} konzipierte Chatbot-System wurde mithilfe der 
...
eingehende Nachrichten verarbeiten und Daten zwischen WhatsApp, Airtable, 
Google Docs und Google Drive austauschen.

Die Grundlage der Kommunikation bildet die \textbf{WhatsApp Business Cloud \gls{ac:api}}. 
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{make_chatbot_szenario.png}
\caption{Hauptszenario des Chatbots in Make.com: WhatsApp-Trigger $\rightarrow$ Router $\rightarrow$ Airtable/Google-Integration $\rightarrow$ Antwort.}
\label{fig:make_chatbot}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{make_router_current_step.png}
\caption{Router-Modul mit current\_step-Steuerung: Verzweigung nach Dialogzustand.}
...
Nach Auswahl oder Neuanlage eines Kunden werden die zugehörigen Stammdaten in \texttt{Customers} gespeichert und zugleich als Relation in \texttt{Rechnungen} übernommen. Der folgende Dialog sammelt schrittweise rechnungsbezogene Informationen (Datum, Rechnungsnummer, Leistungsbeschreibung, Menge, Einzelpreis, Zahlungsfrist, Zahlart). Die Dialogschritte sind durch \texttt{current\_step}-Werte (z.\,B. \emph{Datum ausgewählt}, \emph{Position erstellt}) klar strukturiert. Eingaben werden jeweils kurz bestätigt, bevor das Szenario in den nächsten Schritt wechselt.
...
Nach der Positionsbestätigung fragt der Bot nach Menge und Einzelpreis. Beide Werte werden verifiziert, in temporären Feldern von \texttt{User\_Sessions} zwischengespeichert und anschließend in den Tabellen \texttt{Rechnungen} und \texttt{Rechnungspositionen} persistiert. Abschließend erstellt das Szenario mit der Google-Docs-Integration ein Rechnungsdokument auf Basis eines Templates, fügt alle dynamischen Felder ein, exportiert das Dokument als \gls{ac:pdf} und sendet es dem Nutzer direkt über WhatsApp zurück \citep{make_google_docs}. Damit ist der End-to-End-Prozess der Rechnungserstellung abgeschlossen.
...
\texttt{User\_Sessions}, \texttt{Customers}, \texttt{Rechnungen} und 
\texttt{Rechnungspositionen}. Die Tabelle \texttt{User\_Sessions} dient als 
technischer Sitzungs- und Zwischenspeicher für den Chatbot.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{airtable_user_sessions.png}
\caption{User\_Sessions-Tabelle mit current\_step-Steuerung und Telefonnummern.}
\label{fig:airtable_sessions}
\end{figure}
...
Die Tabelle \texttt{Rechnungen} speichert kopfbezogene Rechnungsdaten wie 
...
Tabelle \texttt{Rechnungspositionen} abgelegt. Diese enthalten neben den zur 
...
\texttt{Rechnungen} und \texttt{Rechnungspositionen} überführt. Auf diese Weise 
...
Airtable stellt hierfür eine \gls{ac:http}-basierte \gls{ac:rest}-\gls{ac:api} bereit, die sowohl in 
Make als auch in serverseitigen Komponenten über \gls{ac:api}-Keys und 
...
Die Business-Logik zur Kommunikation mit der Airtable-\gls{ac:api} und der 
Google-Drive-\gls{ac:api} ist dabei in zwei Edge Functions gekapselt. Die Funktion 
...
Infrastruktur ausgeführt und eignen sich insbesondere für \gls{ac:http}-basierte 
Integrationen mit Drittsystemen 
\citep{supabase_edge_functions}. Innerhalb der 
Funktionen werden notwendige \gls{ac:api}-Schlüssel und Zugriffstoken als Secrets 
verwaltet, sodass sie weder im Browser noch im Frontend-Code sichtbar sind.

Die Edge Functions nehmen \gls{ac:http}-Anfragen
des Frontends entgegen, führen
Authentifizier-ungs- und Autorisierungsprüfungen durch und rufen anschließend
...
\gls{ac:api}-Schlüsseln und der Zugriff ausschließlich durch authentifizierte Nutzer,
erleichtert.
...
\citep{google_drive_api_guide}.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{clienthub_ui.png}
\caption{ClientHub Web-App: Kundenübersicht mit Such-/Filterfunktion.}
\label{fig:clienthub_ui}
\end{figure}

Diese Funktionen unterstützen insbesondere 
...
Kanäle auf die gemeinsamen Tabellen \texttt{Customers}, \texttt{Rechnungen} 
und \texttt{Rechnungspositionen} zugreifen, bleiben Stamm- und Belegdaten 
...
Nach Abschluss des Chat-Dialogs, in dem sämtliche für die Rechnungserstellung erforderlichen Informationen erfasst wurden, erzeugt das System automatisiert ein formatiertes Rechnungsdokument auf Grundlage einer Google-Docs-Vorlage \citep{make_google_docs}. Anschließend wird das Dokument programmgesteuert als \gls{ac:pdf} exportiert und dem Nutzer unmittelbar im WhatsApp-Chat bereitgestellt, sodass keine manuelle Nachbearbeitung in einem Textverarbeitungsprogramm erforderlich ist \citep{google_docs_api}.
...
aus den Airtable-Tabellen \texttt{Customers} und \texttt{Rechnungen} ersetzt
werden.
...
dynamisch befüllt werden können \citep{google_docs_mailmerge}.
\end{sloppypar}

Das zugrunde liegende Platzhalterkonzept entspricht dem in der
Google-Docs-\gls{ac:api} beschriebenen Mail-Merge-Ansatz, bei dem Textmarken in einem
...
\subsection{Generierung und Versand der \texorpdfstring{\gls{ac:pdf}}{PDF}-Rechnung}
...
\texttt{Customers}, \texttt{Rechnungen} und \texttt{Rechnungspositionen}
...
Im nächsten Schritt wird das erzeugte Dokument über die Google-Docs- beziehungsweise Google-Drive-Schnittstelle serverseitig in das \gls{ac:pdf}-Format konvertiert und als Datei bereitgestellt \citep{google_docs_api,google_docs_pdf_automation}. Die resultierende \gls{ac:pdf}-Rechnung wird in der aktuellen Implementierung nicht automatisch in einer strukturierten Ordnerhierarchie in Google Drive abgelegt, sondern primär für den unmittelbaren Versand über den WhatsApp-Chat verwendet. Hierzu wird die \gls{ac:pdf}-Datei über das WhatsApp-Sendemodul an den Nutzer übertragen, der das Dokument lokal speichern oder weiterleiten kann. Eine optionale, langfristige Ablage in Google Drive erfolgt erst in einem separaten Prozessschritt über den Menüpunkt \glqq Dokument speichern\grqq{} im Chatbot \citep{make_whatsapp_docs,google_drive_api_guide}.



\section{Ablagestrategie in Google Drive (Jahr/Quartal/Monat)}

Die Ablage von Dokumenten in Google Drive erfolgt nach einer klaren, zeitbasierten Ordnerstruktur, um Belege auch bei wachsendem Datenvolumen schnell auffindbar und revisionssicher zu verwalten. Alle vom Nutzer hochgeladenen Dokumente werden im Pfad \glqq Dokument speichern\grqq{} des Chatbots systematisch nach Jahr, Quartal und Monat einsortiert.
...
vorteilhaft ist. Erst danach wird der Nutzer
aufgefordert, das betreffende Dokument (z.\,B. eine Rechnung im \gls{ac:pdf}-Format)
zu übermitteln.

Nach Eingang der Datei prüft das Szenario schrittweise die Google-Drive-Ordnerhierarchie (Telefonnummer → Jahr → Quartal → Monat) und legt fehlende Ebenen automatisch an \citep{google_drive_org_guide}. Der kundenbezogene Stammordner (Telefonnummer) wird identifiziert bzw. erstellt, gefolgt von den zeitbasierten Unterordnern für Jahr, Quartal und Monat nach demselben iterativen Muster.


Auf diese Weise entsteht eine konsistente Pfadstruktur der Form:
\begin{center}
\texttt{/Kunden/\textless Telefonnummer\textgreater/\textless Jahr\textgreater/\textless Quartal\textgreater/\textless Monat\textgreater}
\end{center}
ohne dass bereits bestehende Ordner dupliziert werden.
...
beschrieben, wird die Rechnung zunächst als \gls{ac:pdf} generiert und direkt im
...
er erneut den Pfad \glqq Dokument speichern\grqq{} auf und lädt die \gls{ac:pdf}-Datei
...
\label{subsec:clienthub-frontend}
...
\textbf{Views:}
\begin{itemize}
\item \texttt{DashboardView}: StatsCards (Kunden/Dokumente).
\item \texttt{CustomerTable}: \gls{ac:crud}-Tabelle (Prototyp: Read/Create).
\item \texttt{FolderBrowser}: Rekursiver Drive-Baum ($\leq 6$ Ebenen).
\item \texttt{ConnectionsView}: \gls{ac:oauth}-Setup.
\end{itemize}

\textbf{Stack:}
\begin{table}[H]
\centering
\caption{ClientHub Frontend}
\label{tab:clienthub-frontend}
\begin{tabular}{ll}
\hline
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{clienthub_dashboard.jpg}
\caption{ClientHub Dashboard mit Kunden- \& Drive-Übersicht.}
\label{fig:clienthub-ui}
\end{figure}

\subsection{Edge Functions in Lovable Cloud}
\label{subsec:edge-functions}

Zwei Edge Functions auf Basis von \gls{ac:deno} und \gls{ac:ts} kapseln sicherheitskritische Logik und schützen dabei insbesondere \glspl{ac:api}-Zugangsdaten (Airtable und Google Drive).

\begin{table}[H]
\centering
\caption{Edge Functions}
\label{tab:edge-functions}
\begin{tabular}{ll}
\hline
\textbf{Name} & \textbf{Funktionen} \\
\hline
\texttt{airtable-customers} & GET/POST (Base: appN3KaFLqeV4FURN) \\
\texttt{google-drive} & \gls{ac:oauth}, Folders/Tree (v3) \\
\hline
\end{tabular}
\end{table}
Config: \texttt{verify\_jwt=false} (Prototyp). Secrets: Keys/\gls{ac:oauth}.
...
\subsection{\gls{ac:api}-Anbindung an Airtable und Google Drive}
\label{subsec:api-anbindung}

Frontend $\rightarrow$ Edge $\rightarrow$ \glspl{ac:api}
 (\gls{ac:cors}/Secrets serverseitig).

\textbf{Airtable:} Table \texttt{tbljfpEqd4bbEYUqB} (Firmenname, E-Mail, Adresse). Vollständige Liste (Prototyp, kein serverseitiges Filter).

\textbf{Google Drive:} Authorization Code Flow:
\begin{itemize}
\item Frontend: Authorization URL $\rightarrow$ Code empfangen.
\item Edge: Code + Secret $\rightarrow$ Token (\texttt{oauth2.googleapis.com/token}).
\item Drive \gls{ac:api}: \texttt{drive/v3/files} (Ordner/Dateien).
\end{itemize}
...
Da Chatbot und Web-Anwendung auf dieselbe Airtable-Base zugreifen, bleiben Kunden- und Belegdaten kanalübergreifend konsistent.
Die technische Umsetzung bestätigt die Machbarkeit der Konzeption (Kapitel 4). 
Die Evaluation der Implementierung folgt in Kapitel 6.