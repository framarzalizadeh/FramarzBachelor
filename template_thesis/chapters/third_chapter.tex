...
Orchestrierungslayer zusammengeführt und automatisiert verarbeitet.\cite{make_whatsapp_watch_events}
...
\end{figure}
Darüber hinaus stehen verschiedene Sichten und Filter zur Verfügung, mit denen Teilmengen der Daten anhand frei definierbarer Kriterien angezeigt werden können. Formelfelder ermöglichen zudem die Berechnung dynamischer Werte. Sie können unter anderem zur Aggregation von Beträgen oder zur Erzeugung
...
löschen oder anhand definierter Filterkriterien abzufragen. Auf diese Weise kann Airtable als zentrale „Single Source of Truth“ für fachliche Informationen fungieren. Unterschiedliche Frontends und Automatisierungsprozesse greifen dabei über die \gls{ac:api} auf denselben
...
über differenzierte Freigabemechanismen geteilt werden. Über die Google-Drive-\gls{ac:api} lassen sich Ordner und Dateien zudem programmgesteuert anlegen, verschieben, umbenennen dder herunterladen. Dadurch können Dokumentenerzeugung und Archivierung in einen
...
sondern über \emph{Supabase Edge Functions}. Diese Functions sind serverlose \gls{ac:http}-Endpunkte, die bei Bedarf 
...
Frontend offengelegt werden müssen.
...
\subsection{Zentrale Logik in den Functions}
Die Implementierung der Functions ist bewusst schlank gehalten und konzentriert sich auf das sichere 
Weiterreichen von Requests sowie die Normalisierung von Response-Daten.

\paragraph{Airtable: Kunden abrufen (Mapping)}
Die Function \texttt{airtable-customers} ruft Datensätze über die Airtable-\gls{ac:api} ab und transformiert die 
...
Die Function \texttt{google-drive} führt den Token-Austausch im \gls{ac:oauth}-2.0-Prozess serverseitig aus, um 
...
\begin{itemize}
  ...
  \item \textbf{\gls{ac:cors}-Entkopplung:} Direkte \gls{ac:api}-Calls aus dem Browser sind häufig durch \gls{ac:cors}-Restriktionen limitiert; die Function fungiert als kontrollierter Proxy.
  \item \textbf{Serverless-Betrieb:} Es ist kein eigener Server und kein klassisches Deployment/Scaling notwendig, da die Functions on-demand ausgeführt werden.
